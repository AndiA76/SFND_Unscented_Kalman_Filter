...
// Functions and structs used to simulate Lidar raytracing of moving car objets
...
// Strcuture to hold a Lidar ray model
struct Ray
{
	// Initialize internal variables of the Lidar ray model
	Vect3 origin;
	double resolution;
	Vect3 direction;
	Vect3 castPosition;
	...
	// Set the Lidar model with arguments
	...
	void rayCast(const std::vector<Car>& cars, double minDistance, double maxDistance, pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud, double slopeAngle, double sderr)
	{
		...
		// Do not consider any Lidar rays hitting the ego car itself
...
// Structure to hold the model of a Lidar sensor for object detection
struct Lidar
{
	...
	// Initialize a Lidar sensor object
